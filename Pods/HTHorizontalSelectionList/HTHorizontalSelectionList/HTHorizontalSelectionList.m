...
@interface HTHorizontalSelectionList () <UICollectionViewDataSource>
...
- (void)makeCurrentSelectionVisible {
    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:self.selectedButtonIndex inSection:0];
    [self.collectionView scrollToItemAtIndexPath:indexPath atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:YES];
    
    if (self.selectedButtonIndex >= 0 && self.selectedButtonIndex < [self.dataSource numberOfItemsInSelectionList:self]) {
        UICollectionViewLayoutAttributes *attributes = [self.collectionView layoutAttributesForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedButtonIndex inSection:0]];
        CGRect cellRect = attributes.frame;
//        self.selectionIndicatorBar.frame = CGRectMake(cellRect.origin.x+cellRect.size.width+kHTHorizontalSelectionListHorizontalMargin,
...
//                                                      kHTHorizontalSelectionListSelectionIndicatorHeight);
        self.selectionIndicatorBar.frame = CGRectMake(cellRect.origin.x, self.contentView.frame.size.height - kHTHorizontalSelectionListSelectionIndicatorHeight, cellRect.size.width, kHTHorizontalSelectionListSelectionIndicatorHeight);
    }
}
